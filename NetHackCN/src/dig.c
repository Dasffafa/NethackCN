...
mkcavearea(boolean rockit)
{
    ...
    if (rockit) {
        ...
        pline("一种神秘的力量在你周围%s洞穴!",
              (levl[u.ux][u.uy].typ == CORR) ? "创造了一个" : "扩大了那个");
    }
...
{
    const char *verb =
        (madeby == BY_YOU && uwep && is_axe(uwep)) ? "切削" : "挖掘";
...
dig(void)
{
    ...
    const char *verb = (!uwep || is_pick(uwep)) ? "挖掘进去" : "挖穿";
    ...
    if (Fumbling && !rn2(3)) {
        ...
        case 0:
            if (!welded(uwep)) {
                ...
                if (u.usteed)
                    pline("%s并%s %s的腿!", Yobjnam2(uwep, "反弹"),
                          otense(uwep, "打中"), mon_nam(u.usteed));
                else
                    pline("哎哟!  %s并%s你的腿!", Yobjnam2(uwep, "反弹"),
                          otense(uwep, "打中"));
        ...
        if (svc.context.digging.effort <= 50
            ...
            if (rnl(7) > (Fumbling ? 1 : 4)) {
                ...
                if (dmg < 1)
                    ...
                    dmg = (dmg + 1) / 2;
                You("打中你自己的%s.", body_part(FOOT));
                Sprintf(kbuf, "砍掉了 %s自己的 %s", uhis(),
                        body_part(FOOT));
...
digactualhole(coordxy x, coordxy y, struct monst *madeby, int ttyp)
{
    ...
    if (madeby_u) {
        if (x != u.ux || y != u.uy)
            ...
            You("挖了一个坑%s%s.", in_thru, surface_type);
    } else if (!madeby_obj && canseemon(madeby)) {
        pline("%s挖%s%s%s.", Monnam(madeby), an(tname), in_thru,
              surface_type);
    } else if (cansee(x, y) && flags.verbose) {
        pline("%s出现在%s.", An(tname), surface_type);
    }
    if (IS_FURNITURE(old_typ) && cansee(x, y))
        pline_The("%s掉入%s中!", furniture, tname);
...
dighole(boolean pit_only, boolean by_magic, coord *cc)
{
    ...
    if ((ttmp && (undestroyable_trap(ttmp->ttyp) || nohole))
        ...
        pline_The("这里的%s %s太硬了，很难挖进去.", surface(dig_x, dig_y),
        ...
        if (ttmp && is_pit(ttmp->ttyp)
            && rn2(2)) {
            pline_The("巨石掉落，正好堵住了%s坑.",
                      (dig_x != u.ux || dig_y != u.uy) ? "附近的" : "");
    ...
    /* the following two are here for the wand of digging */
        ...
        pline_The("王座太硬了，很难打碎.");

    } else if (IS_ALTAR(old_typ)) {
        pline_The("祭坛太硬了，很难打碎.");
...
use_pick_axe2(struct obj *obj)
{
    ...
    const char *verbing = ispick ? "挖掘" : "切削";
    ...
    if (u.uswallow && do_attack(u.ustuck)) {
        ...
        if (dig_target == DIGTYP_UNDIGGABLE) {
            ...
            if (trap && trap->ttyp == WEB) {
                ...
                pline("%s 被缠在了蜘蛛网中.", Yobjnam2(obj, ""));
                /* you ought to be able to let go; tough luck */
                /* (maybe `move_into_trap()' would be better) */
                nomul(-d(2, 2));
                multi_reason = "被缠在蜘蛛网里";
                nomovemsg = "你挣脱出来.";
                ...
                if (!ispick) {
                    ...
                    pline("你猛击%s时火花四溅.%s", what,
                          vibrate ? "  斧柄剧烈震动！"
                                  : "");
                    if (vibrate)
                        losehp(Maybe_Half_Phys(2), "砍一件坚硬的东西时的斧柄震动",
                               KILLED_BY);
                    wake_nearby(FALSE);
                } else {
        ...
        if (!u.utrap)
            cant_reach_floor(u.ux, u.uy, FALSE, TRUE);
               ...
               && (!trap || (trap->ttyp != LANDMINE
                             && trap->ttyp != BEAR_TRAP))) {
        pline("%s仅仅刮花了%s.", Yobjnam2(obj, (char *) 0),
              surface(u.ux, u.uy));
        u_wipe_engr(3);
    } else {
        ...
        if (!Hallucination)
            ...
            You_feel("看起来你像是个%s.",
                     (ACURR(A_STR) < 6 || ACURR(A_DEX) < 6
                      ...
                      || ACURR(A_INT) < 6 || ACURR(A_WIS) < 6) ? "下等兵"
                                                               : "上等兵");
    } else {
        if (!Hallucination) {
            You_feel("到一股气流.");
...
zap_dig(void)
{
    ...
    if (u.uswallow) {
        ...
        if (!is_whirly(mtmp->data)) {
            if (digests(mtmp->data))
                You("刺穿了 %s %s 的墙壁!", s_suffix(mon_nam(mtmp)),
        ...
        if (!Is_airlevel(&u.uz) && !Is_waterlevel(&u.uz) && !Underwater) {
            if (u.dz < 0 || On_stairs(u.ux, u.uy)) {
                ...
                if (On_stairs(u.ux, u.uy)) {
                    ...
                    pline_The("光束从 %s 上面反弹，然后打中了 %s.",
                              stway->isladder ? "梯子" : "楼梯",
                              ceiling(u.ux, u.uy));
                }
                You("挖松动了 %s上的一块岩石.", ceiling(u.ux, u.uy));
                pline("它掉落到你的%s上!", body_part(HEAD));
    ...
    if (shopdoor || shopwall)
        pay_for_damage(shopdoor ? "损毁" : "挖进", FALSE);
    return;
}
...
bury_objs(int x, int y)
{
    ...
    if (costly && loss) {
        You("还欠 %s %ld %s 因为你把他的商品埋起来了.", shkname(shkp), loss,
            currency(loss));
    }
}
...
rot_corpse(anything *arg, long timeout)
{
    ...
    if (on_floor) {
        ...
        if (flags.verbose) {
            ...
            Your("%s%s %s 掉了%c", obj == uwep ? "持握的 " : "", cname,
