...
surface(coordxy x, coordxy y)
{
    ...
    if (u_at(x, y) && u.uswallow && is_animal(u.ustuck->data))
        ...
        return digests(u.ustuck->data) ? "胃"
               : enfolds(u.ustuck->data) ? "皮"
                 : "没有这个东西！"; /* can't happen (fingers crossed...) */
        ...
        return "墙壁"; /* 'surface' during Passes_walls */
    ...
    if (Invocation_lev(&u.uz)) {
        ...
        *buf = '\0';
        ...
        /* only report "no portal found" when actually expecting a portal */
                 ...
                 || Is_qstart(&u.uz) || at_dgn_entrance("任务")
                 || Is_knox(&u.uz))
...
query_annotation(d_level *lev)
{
    ...
    if (mptr->custom) {
        ...
        Sprintf(tmpbuf, "用什么备注替换\"%.30s%s\"?", mptr->custom,
...
#endif
    {
        ...
        Snprintf(qbuf, sizeof qbuf, "你想要把%s称为什么?", lbuf);
        getlin(qbuf, nbuf);
    }
...
br_string2(branch *br)
{
    ...
    switch (br->type) {
    case BR_PORTAL:
        return closed_portal ? "封闭的传送门" : "传送门";
    case BR_NO_END1:
        return "连接处";
    case BR_NO_END2:
    ...
    *outbuf = '\0';
    if (mptr->flags.castletune && u.uevent.uheard_tune) {
        ...
        if (u.uevent.uheard_tune == 2)
            ...
            Strcpy(tmp, "5个音符组成的旋律");
        Snprintf(outbuf, bsz, " (演奏%s以关闭或打开吊桥)", tmp);
    }
    return outbuf;
}
...
#define ADD2NTOBUF(nam, var, nam2, var2) \
    do {                                                                \
        if (var && var2) {                                              \
            Sprintf(eos(buf), "%s%s %s%s 以及 %s %s%s", COMMA,           \
    ...
    boolean printdun)
{
    ...
    if (on_level(&u.uz, &mptr->lev))
        Sprintf(eos(buf), " <- 你%s这里.",
                (final <= 0 || (final == 1 && how == ASCENDED)) ? "正在"
    ...
    if (mptr->final_resting_place || final > 0) {
        ...
        for (bp = mptr->final_resting_place; bp; bp = bp->next)
            if (bp->bonesknown || wizard || final > 0)
                ++kncnt;
        if (kncnt) {
            Sprintf(buf, "%s%s", PREFIX, "最后的安息之地");
            add_menu_str(win, buf);
            if (died_here) {
                ...
                (void) strsubst(tmpbuf, "他自己", "你自己");
                (void) strsubst(tmpbuf, "她自己", "你自己");
                (void) strsubst(tmpbuf, "他的", "你的");
                (void) strsubst(tmpbuf, "她的", "你的");
