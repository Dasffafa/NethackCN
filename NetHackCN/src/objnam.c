...
static const struct Jitem Japanese_items[] = {
    { SHORT_SWORD, "胁差" },             // 日本短刀
    { BROADSWORD, "忍者刀" },            // 忍者用直刀
    { FLAIL, "双节棍" },                 // 武术器械
    { GLAIVE, "薙刀" },                  // 日本长柄武器
    { LOCK_PICK, "破锁忍具" },           // 忍者开锁工具
    { WOODEN_HARP, "日本筝" },             // 日本传统弦乐器
    { MAGIC_HARP, "魔法日本筝" },          // 附魔版本
    { KNIFE, "锥" },                   // 日式小刀
    { PLATE_MAIL, "日式挂甲" },              // 日本传统铠甲
    { HELMET, "兜" },                    // 日式头盔
    { LEATHER_GLOVES, "和弓手套" },      // 传统射箭手套
    { FOOD_RATION, "兵粮丸" },            // 忍者行军干粮
    { POT_BOOZE, "清酒" },               // 日本米酒
    { 0, "" }                            // 终止标记
};
...
obj_typename(int otyp)
{
    ...
    if (!actualn)
        actualn = (otyp > 0 && otyp < MAXOCLASSES) ? "通用物品" : "未知物品?";
