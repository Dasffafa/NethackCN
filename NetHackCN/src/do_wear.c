...
static NEARDATA const char see_yourself[] = "看见你自己";
static NEARDATA const char unknown_type[] = "未知类型的%s (%d)";
static NEARDATA const char c_armor[] = "盔甲", c_suit[] = "衣服",
    ...
    case DUNCE_CAP:
        if (uarmh && !uarmh->cursed) {
            if (Blind)
                ...
                pline("%s了片刻%s光芒.", Tobjnam(uarmh, "发出"),
                      hcolor(NH_BLACK));
            curse(uarmh);
        ...
        if (Hallucination) {
            pline("我的大脑很疼!"); /* Monty Python's Flying Circus */
...
armor_or_accessory_off(struct obj *obj)
{
    ...
    if (obj == uskin
        ...
        if (obj != uskin) {
            ...
            Snprintf(why, sizeof(why), "没有先脱下你的%s",
                     what);
        } else {
...
accessory_or_armor_on(struct obj *obj)
{
    ...
    if (armor) {
        ...
        if (ring) {
            ...
            if (!has_head(gy.youmonst.data)) {
                You("没有脑袋用来戴%s.", ansimpleoname(obj));
                return ECMD_OK;
            }
    ...
    if (armor) {
        ...
        if (delay) {
            nomul(delay);
            gm.multi_reason = "穿戴装备";
            gn.nomovemsg = "你完成了你的穿戴动作.";
        } else {
...
*/
    ...
    if (u.twoweap && otmp) {
        ...
        Your("%s %s%s 出你的 %s%s.", otherwep, xfl ? "也 " : "",
    ...
    if (otmp && otmp->otyp != AKLYS && !welded(otmp)) {
        ...
        pline("%s %s%s %s%s 出你的 %s%s.",
              !strncmp(thiswep, "尸体", 6) ? "" : "你",
    ...
    *buf = '\0'; /* lint suppression */
    ...
    if (otmp == uright || otmp == uleft) {
        ...
        if (welded(uwep) && ((otmp == RING_ON_PRIMARY) || bimanual(uwep))) {
            Sprintf(buf, "腾出拿着武器的%s", body_part(HAND));
            why = uwep;
    ...
    if (otmp == uarmg) {
        if (welded(uwep)) {
            ...
            pline("%s%s太滑了脱不下来.",
                  uarmg->unpaid ? "这" : "你的", /* simplified Shk_Your() */
                  gloves_simple_name(uarmg));
            return 0;
        }
    }
...
menu_remarm(int retry)
{
    ...
    n = query_objlist("你想脱掉哪个？", &gi.invent,
...
destroy_arm(struct obj *atmp)
{
    ...
    if ((otmp = maybe_destroy_armor(uarmc, atmp, &resistedc)) != 0) {
        ...
        urgent_pline("你的%s瓦解了!", shield_simple_name(otmp));
    } else {
