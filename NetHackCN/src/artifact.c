...
touch_artifact(struct obj *obj, struct monst *mon)
{
    ...
    if (((badclass || badalign) && self_willed)
        ...
        if (!yours)
            return 0;
        You("被迸发出的%s力量伤到了!", s_suffix(the(xname(obj))));
        touch_blasted = TRUE;
...
dump_artifact_info(winid tmpwin)
{
    ...
    for (m = 1; m <= NROFARTIFACTS; ++m) {
        Snprintf(buf2, sizeof buf2,
                ...
                artiexist[m].exists ? " 存在;" : "",
                artiexist[m].found  ? " 英雄知晓" : "",
                /* .exists and .found have different punctuation because
                   they're expected to be combined with one of these */
                artiexist[m].gift   ? " 馈赠所得"   : "",
                artiexist[m].wish   ? " 许愿所得"   : "",
                artiexist[m].named  ? " 命名所得"  : "",
                artiexist[m].viadip ? " 浸喷泉所得" : "",
                artiexist[m].lvldef ? " 等级" : "",
                artiexist[m].bones  ? " 骨档所得"  : "",
                artiexist[m].rndm   ? " 随机得到" : "");
    ...
    int dieroll)        /* needed for Magicbane and vorpal blades */
{
    ...
    *dmgptr += spec_dbon(otmp, mdef, *dmgptr);
        ...
        if (is_art(otmp, ART_TSURUGI_OF_MURAMASA) && dieroll == 1) {
            ...
            if (!youdefend) {
                ...
                if (bigmonst(mdef->data)) {
                    if (youattack)
                        You("用刀深深划进%s的身体!", mon_nam(mdef));
                    else if (vis)
                        pline("%s深深地切进%s的身体!", Monnam(magr),
                              hittee);
                    *dmgptr *= 2;
                    return TRUE;
                }
    ...
    if (spec_ability(otmp, SPFX_DRLI)) {
        ...
        const char *life = nonliving(mdef->data) ? "活化能量" : "生命";
        ...
        if (!youdefend) {
            ...
            if (Blind) {
                ...
                if (is_art(otmp, ART_STORMBRINGER))
                    ...
                    pline("%s吸取你的%s!", The(otmpname), life);
            }
            losexp("生命吸取");
...
mkot_trap_warn(void)
{
    ...
    if (!uarmg && u_wield_art(ART_MASTER_KEY_OF_THIEVERY)) {
        ...
        if (ntraps != gm.mkot_trap_warn_count) {
            ...
            pline_The("这钥匙感觉%s%c", heat[idx], (ntraps > 3) ? '!' : '.');
        }
