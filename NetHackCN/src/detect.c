...
gold_detect(struct obj *sobj)
{
    ...
    if (!gk.known) {
        ...
        if (gy.youmonst.data == &mons[PM_GOLD_GOLEM])
            Sprintf(buf, "你感觉你就像一个百万%s！", currency(2L));
    ...
    if (stale)
        docrt();
    You("注意到有一些金币在你的两%s之间.", makeplural(body_part(FOOT)));
    return 0;
...
food_detect(struct obj *sobj)
{
    ...
    if (!ct && !ctu) {
        ...
        if (sobj) {
            if (sobj->blessed) {
                Your("%s%s刺痛，然后你闻到%s的味道.", body_part(NOSE),
                     u.uedibility ? "继续" : "开始", what);
                u.uedibility = 1;
            } else
                Your("%s刺痛，然后你闻到%s的味道.", body_part(NOSE), what);
        } else
            You("感觉到%s.", what);
        exercise(A_WIS, TRUE);
...
display_trap_map(int cursed_src)
{
    ...
    You_feel("%s.", cursed_src ? "相当贪婪" : "被困住了");
...
use_crystal_ball(struct obj **optr)
{
    ...
    if (Blind) {
        pline("很糟糕，你看不见那个%s.", the(xname(obj)));
        return;
    }
    ...
    if (charged && (obj->cursed || rnd(oops) > ACURR(A_INT))) {
        ...
        case 1:
            pline("%s传达给你的信息太多了，令你无法理解!", Tobjnam(obj, ""));
            break;
        case 2:
            pline("%s你混乱!", Tobjnam(obj, "令"));
            make_confused((HConfusion & TIMEOUT) + impair, FALSE);
            break;
        case 3:
            if (!resists_blnd(&gy.youmonst)) {
                pline("%s损害你的视力！", Tobjnam(obj, ""));
                make_blinded(BlindedTimeout + impair, FALSE);
                if (!Blind)
                    Your1(vision_clears);
            } else {
                pline("%s试图冲击你的视力.", Tobjnam(obj, ""));
                You("没有受到影响!");
            }
            break;
        case 4:
            pline("%s内的信息灌入你的思想!", Tobjnam(obj, ""));
    ...
    if (Hallucination) {
        ...
        gm.multi_reason = "盯着一个魔法八号球(专利所有)";
        gn.nomovemsg = "";

        if (!charged) {
            pline("你只看到怪异的%s光晕。", hcolor((char *) 0));
            if (obj->spe < 0)
                goto implode; /* 当被取消时销毁它 */
        } else {
            switch (rnd(6)) {
            case 1:
                You("看到一团团发光的熔岩在舞动。");
    ...
    if (flags.verbose)
        You("可以寻找一件物品或者一只怪的符号.");
    ch = yn_function("你要查找什么？", (char *) 0, '\0', TRUE);
    ...
    You("窥视着%s的内部...", the(xname(obj)));
    nomul(-rnd(charged ? 10 : 2));
    gm.multi_reason = "凝视一个水晶球";
    gn.nomovemsg = "";
...
findit(void)
{
    ...
    if (found.num_traps) {
        if (*buf) /* "doors, corrs, and traps" or "{doors|corrs} and traps"
                   * or "..., traps ..." */
            Strcat(buf, (k == 3 && !found.num_mons) ? ", 和"
                        : (k == 2) ? " 和 " : ", ");
    ...
    if (found.num_mons) {
        if (*buf)
            Strcat(buf, (k > 2) ? ", 和 " : " 和 ");
        if (found.num_mons > 1)
            Sprintf(eos(buf), "%d 只隐藏的怪物", found.num_mons);
        else
            Strcat(buf, "一只隐藏的怪物");
        num += found.num_mons;
    }
    ...
    if (found.num_invis) {
        if (found.num_invis > 1)
            Sprintf(buf, "%d只%s未被看见的怪物", found.num_invis,
                    found.num_kept_invis ? " 其它的" : "");
        else
            Sprintf(buf, "%s 未被看见的怪物",
                    found.num_kept_invis ? "另一只" : "一只");
        You("detect %s!", buf);
        num += found.num_invis;
    }
    ...
    if (u.uswallow) {
        if (digests(u.ustuck->data)) {
            ...
            if (Blind)
                ...
                pline("%s 张开血盆大口!", Monnam(u.ustuck));
...
dosearch(void)
{
    if (cmd_safety_prevention("搜索", "继续搜索",
                          "你已经发现了一只怪物.",
    ...
    unsigned which_subset) /* TER_TRP | TER_OBJ | TER_MON | TER_FULL */
{
    ...
    if ((Hallucination || Stunned || Confusion) && !full) {
        ...
        if (full) {
            ...
            Strcpy(buf, "已知地图");
            if (keep_traps)
                Sprintf(eos(buf), "%s 个陷阱",
                        (keep_objs || keep_mons) ? "," : " 和");
            if (keep_objs)
                Sprintf(eos(buf), "%s%s 个物品",
