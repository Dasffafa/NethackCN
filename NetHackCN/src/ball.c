...
ballfall(void)
{
    ...
    if (gets_hit) {
        ...
        losehp(Maybe_Half_Phys(dmg), "被一个铁球砸中脑袋",
               NO_KILLER_PREFIX);
    }
}
...
drop_ball(coordxy x, coordxy y)
{
    ...
    if (x != u.ux || y != u.uy) {
        static const char *pullmsg = "铁球把你拉出了%s!";
        struct trap *t;
        long side;
...
drag_down(void)
{
    ...
    if (forward) {
        ...
        if (rn2(2)) {
            ...
            pline_The("铁球和你撞了个满怀!");
            losehp(Maybe_Half_Phys(rnd(20)), "与铁球的碰撞",
                   KILLED_BY_AN);
