    ...
    if (!rn2(clumsy ? 3 : 4) && (clumsy || !bigmonst(mon->data))
        ...
        if (!nohands(mon->data) && !rn2(martial() ? 5 : 3)) {
            ...
            if (mon->mx != x || mon->my != y) {
                ...
                pline("%s %s, %s回避了你的%s踢击.", Monnam(mon),
    ...
    if (!likes_gold(mtmp->data) && !mtmp->isshk && !mtmp->ispriest
        ...
        if (mtmp->isshk) {
            ...
            if (robbed) {
                ...
                if (robbed < 0L)
                    robbed = 0L;
                pline_The("数目的金币相当于%s弥补了%s最近的损失.",
                          !robbed ? "" : "部分", mhim(mtmp));
...
really_kick_object(coordxy x, coordxy y)
{
    ...
    if (slide && !Blind)
        pline("哦呀! %s%s过%s去了.", Doname2(gk.kickedobj),
...
kickstr(char *buf, const char *kickobjnam)
{
    ...
    if (*kickobjnam)
        ...
        what = "奇怪的东西";
    return strcat(strcpy(buf, "在踢 "), strcat(what,"的时候"));
}
...
kick_nondoor(coordxy x, coordxy y, int avrg_attrib)
{
    ...
    if (IS_TREE(gm.maploc->typ)) {
        ...
        if (rn2(15) && !(gm.maploc->looted & TREE_LOOTED)
            ...
            if (is_plural(treefruit))
                pline("有几个%s从树上掉下来了!", xname(treefruit));
            else
                pline("%s从树上掉下来了!", An(xname(treefruit)));
    ...
    if (IS_SINK(gm.maploc->typ)) {
        ...
        if (rn2(5)) {
            ...
            if (Blind) {
                if (!Deaf)
                    You_hear("有什么东西涌上来的声音.");
            } else {
                pline("一个%s软泥从下水道里涌了上来!",
                      hcolor(NH_BLACK));
            }
...
dokick(void)
{
    ...
    if (nolimbs(gy.youmonst.data) || slithy(gy.youmonst.data)) {
        ...
        if (yn_function("踢你的坐骑?", ynchars, 'y', TRUE) == 'y') {
        ...
        Your("行李太重了，在踢的时候无法保持平衡.");
        no_kick = TRUE;
    } else if (gy.youmonst.data->mlet == S_LIZARD) {
        Your("腿不能有效地踢,");
        no_kick = TRUE;
    } else if (u.uinwater && !rn2(2)) {
        Your("慢动作一样的踢击什么都没有踢中.");
        no_kick = TRUE;
    } else if (u.utrap) {
        no_kick = TRUE;
        switch (u.utraptype) {
    ...
    xint16 dlev)          /* if !0 send to dlev near player */
{
    ...
    if (dct && cansee(x, y)) { /* at least one object fell */
        const char *what = (dct == 1L ? "物品掉" : "许多物品掉");

        if (missile)
            pline("受其影响, 另外的%s%s了下来.",
    ...
    if (costly && shkp && price) {
        if (ESHK(shkp)->robbed > robbed) {
            You("弄没了价值 %ld %s 的物品!", price, currency(price));
            if (cansee(shkp->mx, shkp->my)) {
                if (ESHK(shkp)->customer[0] == 0)
                    (void) strncpy(ESHK(shkp)->customer, svp.plname, PL_NSIZ);
                if (angry)
                    pline("%s勃然大怒!", Shknam(shkp));
                else
        ...
        if (ESHK(shkp)->debit > debit) {
            ...
            You("因为造成的物品损失，欠 %s %ld %s.", shkname(shkp), amt,
                currency(amt));
        }
    }
}
...
otransit_msg(struct obj *otmp, boolean nodrop, boolean chainthere, long num)
{
    ...
    if (num || chainthere) {
        ...
        if (num) { /* means: other objects are impacted */
            ...
            Sprintf(xbuf, "你的铁链%s", otense(otmp, "哗啦作响"));
        }
        if (nodrop)
            Sprintf(eos(xbuf), ".");
        else
        ...
        pline("%s%s%s.", obuf, otense(otmp, "掉"), gg.gate_str);
}
...
down_gate(coordxy x, coordxy y)
{
    ...
    if ((ttmp = t_at(x, y)) != 0 && ttmp->tseen && is_hole(ttmp->ttyp)) {
        gg.gate_str = (ttmp->ttyp == TRAPDOOR) ? "过陷阱门"
                                              : "进洞里";
        return MIGR_RANDOM;
    }
    return MIGR_NOWHERE;
