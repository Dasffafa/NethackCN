...
breakchestlock(struct obj *box, boolean destroyit)
{
    if (!destroyit) { /* bill for the box but not for its contents */
        ...
        pline("事实上, 你彻底摧毁了%s.", the(xname(box)));
        ...
        if (loss)
            You("损坏了价值%ld %s的商品.", loss, currency(loss));
        delobj(box);
    }
}
...
forcelock(void)
{
    ...
    if (gx.xlock.picktyp) { /* blade */
        if (rn2(1000 - (int) uwep->spe) > (992 - greatest_erosion(uwep) * 10)
             ...
             */
            pline("%s%s折断了!", (uwep->quan > 1L) ? "你的一把" : "你的",
                  xname(uwep));
            useup(uwep);
            You("放弃了撬锁的企图.");
            exercise(A_DEX, TRUE);
    ...
    struct obj *container)  /* container, for autounlock */
{
    ...
    if (nohands(gy.youmonst.data)) {
        You_cant("拿住%s -- 你没有手!", doname(pick));
        return PICKLOCK_DID_NOTHING;
    } else if (u.uswallow) {
        You_cant("%s解锁%s.", (picktyp == CREDIT_CARD) ? "" : "上锁或",
                 mon_nam(u.ustuck));
        return PICKLOCK_DID_NOTHING;
    }
    ...
    if (u_at(cc.x, cc.y)) { /* pick lock on a container */
        ...
        if (u.dz < 0 && !autounlock) { /* beware stale u.dz value */
            ...
            pline("那样做的话,%s可能会熔化.", yname(pick));
            return PICKLOCK_LEARNED_SOMETHING;
        } else if (is_pool(u.ux, u.uy) && !Underwater) {
            pline_The("%s上面没有锁.", hliquid("水"));
            return PICKLOCK_LEARNED_SOMETHING;
        }
        ...
        for (otmp = svl.level.objects[cc.x][cc.y]; otmp;
            ...
            if (Is_box(otmp)) {
                ...
                if (autounlock && (flags.autounlock & AUTOUNLOCK_UNTRAP) != 0
                    ...
                    c = 'q';
                    if (pick != &dummypick) {
                        Sprintf(qbuf, "用%s来撬锁?", yname(pick));
                        c = ynq(qbuf);
                    }
                    if (c != 'y')
                        return PICKLOCK_DID_NOTHING;
...
doforce(void)
{
    ...
    if (u.uswallow) {
        You_cant("在一个生物的肚子里撬锁.");
        return ECMD_FAIL;
    }
    if (!u_have_forceable_weapon()) {
    boolean use_plural = uwep && uwep->quan > 1;
    const char *weapon_desc = NULL;
    
    if (!uwep) {
        weapon_desc = "没有装备任何武器时";
    } else if (uwep->oclass != WEAPON_CLASS && !is_weptool(uwep)) {
        weapon_desc = use_plural ? "使用这些不合适的工具" : "使用这件不合适的工具";
    } else {
        weapon_desc = use_plural ? "用这些武器" : "用这件武器";
    }

    You_cant("在%s撬锁。", weapon_desc);
    return ECMD_OK;
}
    if (!can_reach_floor(TRUE)) {
    ...
    for (otmp = svl.level.objects[u.ux][u.uy]; otmp; otmp = otmp->nexthere)
        if (Is_box(otmp)) {
            ...
            if (picktyp)
                You("把%s 伸进缝隙中然后使劲地撬动.", yname(uwep));
            else
...
doorlock(struct obj *otmp, coordxy x, coordxy y)
{
    ...
    const char *quickly_dissipates = "很快消散了";
...
chest_shatter_msg(struct obj *otmp)
{
    ...
    if (otmp->oclass == POTION_CLASS) {
        You("%s%s粉碎了!", Blind ? "听见" : "看见", an(bottlename()));
