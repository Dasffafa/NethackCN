...
done2(void)
{
    ...
    if (abandon_tutorial || !paranoid_query(ParanoidQuit, "确定退出?")) {
#ifndef NO_SIGNAL
        (void) signal(SIGINT, (SIG_RET_TYPE) done1);
#endif
        clear_nhwindow(WIN_MESSAGE);
...
done_in_by(struct monst *mtmp, int how)
{
    ...
    if (imitator) {
        ...
        if (has_mgivenname(mtmp))
            Sprintf(eos(buf), "之%s", MGIVENNAME(mtmp));
    } else if (mtmp->isshk) {
        const char *shknm = shkname(mtmp),
                   *honorific = shkname_is_pname(mtmp) ? ""
                                   : mtmp->female ? "Ms. " : "Mr. ";

        Sprintf(eos(buf), "%s%s店主", honorific, shknm);
        svk.killer.format = KILLED_BY;
        ...
        if (has_mgivenname(mtmp))
            Sprintf(eos(buf), "叫做%s", MGIVENNAME(mtmp));
    }
...
disclose(int how, boolean taken)
{
    ...
    if (!done_stopprint) {
        ...
        c = ask ? yn_function("你想查看你的各项属性吗?", ynqchars,
                              defquery, TRUE)
                : defquery;
        if (c == 'y')
            enlightenment((BASICENLIGHTENMENT | MAGICENLIGHTENMENT),
...
really_done(int how)
{
    ...
    if (how == QUIT) {
        ...
        if (u.uhp < 1) {
            ...
            Strcpy(svk.killer.name, "行将就木之时退出游戏");
        }
    }
...
#endif
    if (u.uhave.amulet) {
        Strcat(svk.killer.name, " (带着岩德护身符)");
    } else if (how == ESCAPED) {
    ...
    if (how == ESCAPED || how == ASCENDED) {
        ...
        if (u.uz.dnum == 0 && u.uz.dlevel <= 0) {
            ...
            if (!In_endgame(&u.uz) && !single_level_branch(&u.uz))
                Sprintf(eos(pbuf), "在地牢的第%d层, ",
                        In_quest(&u.uz) ? dunlev(&u.uz) : depth(&u.uz));
        }

        Sprintf(eos(pbuf), "有%ld分数,", u.urexp);
        dump_forward_putstr(endwin, 0, pbuf, done_stopprint);
    }

    Sprintf(pbuf, "%ld金币, 移动了%ld步.", umoney,
            g.moves);
    dump_forward_putstr(endwin, 0, pbuf, done_stopprint);
    Sprintf(pbuf,
            "在你%s的时候, 你的等级为%d, 最大生命值为%d.",
