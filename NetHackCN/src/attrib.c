...
static const struct innate {
  ...
  kni_abil[] = { { 7, &(HFast), "行动迅速", "行动迟缓" }, { 0, 0, 0, 0 } },
  ...
  mon_abil[] = { { 1, &(HFast), "", "" },
                 ...
                 { 5, &(HStealth), "行踪隐秘", "" },
                 { 7, &(HWarning), "敏感", "" },
                 { 9, &(HSearching), "富有洞察力", "注意力不集中" },
                 { 11, &(HFire_resistance), "凉爽", "更热" },
                 { 13, &(HCold_resistance), "温暖", "更冷" },
                 { 15, &(HShock_resistance), "绝缘", "导电" },
                 { 17, &(HTeleport_control), "受控", "失控" },
                 { 0, 0, 0, 0 } },

  pri_abil[] = { { 15, &(HWarning), "敏感", "" },
                 { 20, &(HFire_resistance), "凉爽", "更热" },
                 { 0, 0, 0, 0 } },
...
{               /* negative => conditional (msg if change made) */
    ...
    disp.botl = TRUE;
    if (msgflg <= 0)
        You_feel("变得%s%s!", (incr > 1 || incr < -1) ? "非常 " : "", attrstr);
    ...
    int reason) /* A_CG_CONVERT, A_CG_HELM_ON, or A_CG_HELM_OFF */
{
    ...
    if (reason == A_CG_CONVERT) {
        ...
        livelog_printf(LL_ALIGNMENT, "永久转化为%s",
