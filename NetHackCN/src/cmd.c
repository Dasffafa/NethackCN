...
const char unavailcmd[] = "不可用的命令'%s'.";

/* for rejecting #if !SHELL, !SUSPEND */
static const char cmdnotavail[] = "不可用的命令'%s'.";
...
struct ext_func_tab extcmdlist[] = {
    ...
    { 'd',    "drop", "丢弃一件物品",
              dodrop, 0, NULL },
    { 'D',    "droptype", "丢弃特定类别的物品",
              doddrop, 0, NULL },
    { 'e',    "eat", "吃东西",
              doeat, CMD_M_PREFIX, NULL },
    { 'E',    "engrave", "在地板上写字",
              doengrave, 0, NULL },
    { M('e'), "enhance", "查看或者提升你的武器和施法技能",
    ...
    { 'v',    "versionshort", "显示版本和程序构建的日期+时间",
    ...
    { '\0',   "wizloaddes", "加载并执行一个des-file lua脚本",
    ...
    { 'z',    "zap", "使用一根魔杖",
              dozap, 0, NULL },
    /* 移动命令将由reset_commands()绑定 */
    { '\0',   "movewest", "向西移动(屏幕左侧)",
              do_move_west, MOVEMENTCMD | CMD_MOVE_PREFIXES, NULL },
    { '\0',   "movenorthwest", "向西北移动(屏幕左上)",
...
handler_rebind_keys_add(boolean keyfirst)
{
    ...
    if (key) {
        if (gc.Cmd.commands[key]) {
            Sprintf(buf, "按键'%s' 现在被绑定到 \"%s\".",
                    key2txt(key, buf2), gc.Cmd.commands[key]->ef_txt);
        } else {
            Sprintf(buf, "按键'%s'现在没有任何绑定.",
                    key2txt(key, buf2));
        }
    ...
    if (key)
        Sprintf(buf, "绑定'%s' 到哪个命令?", key2txt(key, buf2));
    else
        Sprintf(buf, "绑定哪个命令?");
    end_menu(win, buf);
